...
main <- function() {
    optionList <- list(
        ...
        optparse::make_option(c("-rl", "--rsids_line"),
            ...
            help = paste0(
                "List of rsids (e.g., rs516946,rs508419) .",
                "In the final plot, if the rsid in LD with the tag variant, ",
                "there will be vertical line highlighting each variant listed ",
                "here. ",
                "[default %default]"
            )
        ),
